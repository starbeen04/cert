<template>
  <div class="instructor-layout" :class="{ 'sidebar-collapsed': isCollapsed }">
    <!-- Sidebar -->
    <nav class="sidebar" :class="{ 'collapsed': isCollapsed }">
      ...
      >
        <el-menu-item
          v-for="route in instructorMenuRoutes"
          :key="route.path"
          :index="route.path"
          @click="navigateTo(route.path)"
        >
          <el-icon>
            <component :is="getIconComponent(route.meta?.icon || 'Menu')" />
          </el-icon>
...
// 강사 메뉴 라우트들
const instructorMenuRoutes = computed(() => [
  {
    path: '/instructor',
...
const userInitials = computed(() => {
  ...
  return user?.username?.charAt(0).toUpperCase() || 'I'
})
...
const getIconComponent = (iconName: string) => {
  ...
  return icons[iconName as keyof typeof icons] || Menu
}
...
.instructor-layout.sidebar-collapsed {
